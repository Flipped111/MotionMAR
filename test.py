...
def load_var(args):
    ...
    ckpt = torch.load(var_ckpt, map_location=device)
    if "epoch" in ckpt:
        print(f"Loaded checkpoint at epoch: {ckpt['epoch']}")
    temp2 = ckpt["trainer"]["var_wo_ddp"]
...
def overlapping_test_simplify(args, data, model, inference_kwargs, num_per_batch=16):
...
def test_process():
    args = get_args(include_inference=True)
    ...
    for sample_index in tqdm(range(len(dataset))):
        output, body_param, head_motion, filename = \
            overlapping_test_simplify(args, dataset[sample_index], model, inference_kwargs, n_testframe)
