...
class Args(Tap):
    ...
    data_path: str = "VQVAE/dataset"
    exp_name: str = 'text'
    
    # Add new
    
    PROTOCOL: str= "1_small_dataset"
    INPUT_MOTION_LENGTH: int = 20
    TRAIN_DATASET_REPEAT_TIMES: int= 1000
    sparse_dim: int = 54
    num_classes: int = 18*3*20
    ...
    pn: str = '5_10_20'
    patch_size: int = 20
    patch_nums: tuple = (5, 10, 20)   # [automatically set; don't specify this] = tuple(map(int, args.pn.replace('-', '_').split('_')))
    resos: tuple = None         # [automatically set; don't specify this] = tuple(pn * args.patch_size for pn in args.patch_nums)
    vae_ckpt: str = 'outputs/S1/vqvae/best.pth.tar'
    vae_vocab_size: int = 1024
    vae_code_dim: int = 64
    vqvae_layers: str = '4_4'
    vqvae_hid_dim: int = 512
    vqvae_heads: int = 4
    vqvae_dropout: float = 0.0
    vqvae_n_codebook: int = 8
    vqvae_beta: float = 1.0
    ...
    pg: float = 0.0         # >0 for progressive training during the initial fraction of training
    ...
    branch: str = '[unknown]'  # [automatically set; don't specify this]
    commit_id: str = '[unknown]'  # [automatically set; don't specify this]
    commit_msg: str = '[unknown]'  # [automatically set; don't specify this]
    ...
    # environment
    local_out_dir_path: str = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'outputs', 'S1', 'var')  # [automatically set; don't specify this]
    ...
    device: str = None     # [automatically set; don't specify this]
    seed: int = None        # seed
...
def init_dist_and_get_args():
    ...
    args.resos = tuple(args.patch_nums)
    ...
    args.last_ckpt_path = os.path.join(args.local_out_dir_path, 'best.pth.tar')
