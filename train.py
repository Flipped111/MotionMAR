...
from VQVAE.dataloader.dataloader import load_data, TrainDataset
...
def _parse_layers(layers):
    ...
    return tuple(int(x) for x in str(layers).replace("-", "_").split("_"))
...
def build_everything(args: arg_util.Args):
    ...
    if not args.local_debug:
        ...
        val_motions, val_sparses, val_info= load_data(
        args.data_path,
        "val",
        protocol=args.PROTOCOL,
        input_motion_length=args.INPUT_MOTION_LENGTH,
        ...
        # Distributed sampler for the validation set.
        ld_val = torch.utils.data.DataLoader(
            val_dataset,
            num_workers=0,
            pin_memory=True,
            batch_size=round(args.batch_size * 1.5),
            sampler=EvalDistributedSampler(val_dataset, num_replicas=dist.get_world_size(), rank=dist.get_rank()),
            shuffle=False,
            drop_last=False,
        )
    ...
    vae_local, var_wo_ddp = build_vae_var(
        V=args.vae_vocab_size, Cvae=args.vae_code_dim, share_quant_resi=4,
        vqvae_n_layers=_parse_layers(args.vqvae_layers),
        vqvae_hid_dim=args.vqvae_hid_dim,
        vqvae_heads=args.vqvae_heads,
        vqvae_dropout=args.vqvae_dropout,
        vqvae_n_codebook=args.vqvae_n_codebook,
        vqvae_beta=args.vqvae_beta,
        sparse_input_dim=args.sparse_dim,
    ...
    trainer = VARTrainer(
        device=device, patch_nums=args.patch_nums, resos=args.resos,
...
def main_training():
    ...
    for ep in range(start_ep, args.ep):
        ...
        if is_val_and_also_saving:
            ...
            # Only the local master saves to avoid multi-process write collisions.
            if dist.is_local_master():
                local_out_ckpt = os.path.join(args.local_out_dir_path, 'best.pth.tar')
                ...
                if (ep + 1) % 50 == 0:
                    local_out_ckpt_ep = os.path.join(args.local_out_dir_path, f'ar-ckpt-ep{ep + 1:04d}.pth')
...
def train_one_ep(ep: int, is_first_ep: bool, start_it: int, args: arg_util.Args, tb_lg: misc.TensorboardLogger, ld_or_itrt, iters_train: int, trainer):
    ...
    for it, (inp, label) in me.log_every(start_it, iters_train, ld_or_itrt, 30 if iters_train > 8000 else 5, header):
        ...
        grad_norm, scale_log2 = trainer.train_step(
            ...
            inp_BCL=inp, label_B=label, prog_si=prog_si, prog_wp_it=args.pgwp * iters_train,
        )
    ...
    return {k: meter.global_avg for k, meter in me.meters.items()}, me.meters['iter_time'].time_preds(max_it - (g_it + 1) + (args.ep - ep) * 15)  # +15: other cost
